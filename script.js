...
async function loadStripeLinks() {
  ...
  for (const [name, item] of Object.entries(data)) {
    let imgHtml = '';
    if (item.image) {
      imgHtml = `<img src="${item.image}" alt="${name}" style="width: 100%; height: auto; max-height: 280px; object-fit: contain; border-radius: 4px; margin-bottom: 16px; display: block; background: #000;">`;
    }
    ...
    html += `
      <article class="card card-live" style="display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          <h3>${name}</h3>
          ${imgHtml}
          <p class="card-meta" style="margin-bottom: 20px;">${desc}</p>
        </div>
